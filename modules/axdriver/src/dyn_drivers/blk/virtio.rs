...
use rdrive::{
    module_driver, probe::OnProbeError, register::FdtInfo, DriverGeneric, PlatformDevice,
};
...
impl rdif_block::IQueue for BlockQueue {
    ...
    fn poll_request(&mut self, _request: rdif_block::RequestId) -> Result<(), rdif_block::BlkError> {
        Ok(())
    }
}
