...
use rk3568_driver_block::{set_impl, Kernel, EmmcDriver, EMmcHost, init_clk};
use rk3568_driver_clk::ClkDriver;

use somehal::{
    driver::{
        register::FdtInfo, intc::Box, Descriptor, 
        HardwareKind
    },
    module_driver,
};
...
fn probe_mmc(info: FdtInfo<'_>, _dev: &Descriptor) -> Result<HardwareKind, Box<dyn Error>> {
    ...
    Ok(HardwareKind::Block(Box::new(EmmcDriver(
        emmc,
    ))))
}
...
fn probe_clk(info: FdtInfo<'_>, _dev: &Descriptor) -> Result<HardwareKind, Box<dyn Error>> {
    ...
    Ok(HardwareKind::Clk(Box::new(
        ClkDriver::new(cru_address)
    )))
}
...
impl BlockDriverOps for Block {
    ...
    fn flush(&mut self) -> DevResult {
        self.0.spin_try_borrow_by(0.into()).unwrap().flush().map_err(convert_error)
    }

    fn read_block(&mut self, block_id: u64, buf: &mut [u8]) -> DevResult {
        self.0.spin_try_borrow_by(0.into()).unwrap().read_block(block_id, buf).map_err(convert_error)
    }

    fn write_block(&mut self, block_id: u64, buf: &[u8]) -> DevResult {
        self.0.spin_try_borrow_by(0.into()).unwrap().write_block(block_id, buf).map_err(convert_error)
    }
}
